...
export default async function services(data) {
    const { libreria, tokens, id_chat } = data;
    if (!libreria || !tokens || !id_chat) {
        let infoError= "Se encontró el sigiente campo vacío: " + (!libreria ? " [libreria] " : "" )+ (!tokens ? " [tokens] " : "") + (!id_chat ? " [id_chat] " : "")
